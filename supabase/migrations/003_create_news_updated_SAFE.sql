-- ============================================================
-- SAFE MIGRATION: Create news_updated table
-- ============================================================
-- This migration is SAFE to run. It does NOT:
-- - Create or modify existing buckets (news-images, news-attachments)
-- - Affect existing news table
-- - Create conflicting constraints or policies
-- ============================================================

-- 1. Create news_updated table with multi-image and rich content support
...
-- 2. Create indexes for JSONB columns (better query performance)
...
CREATE INDEX IF NOT EXISTS idx_news_updated_user_id ON public.news_updated(user_id);
CREATE INDEX IF NOT EXISTS idx_news_updated_created_at ON public.news_updated(created_at DESC);

-- 3. Enable Row Level Security
ALTER TABLE public.news_updated ENABLE ROW LEVEL SECURITY;

-- 4. Drop existing policies if any (to avoid conflicts on re-run)
...
-- Admins can view all news (using is_admin field from profiles)
...
-- 7. Drop existing trigger if any (to avoid conflicts)
...
-- 10. Verify storage buckets exist (just a check, will not create)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM storage.buckets WHERE id = 'news-images') THEN
    RAISE NOTICE 'WARNING: Storage bucket "news-images" does not exist. Please create it manually.';
  ELSE
    RAISE NOTICE 'Storage bucket "news-images" exists: OK';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM storage.buckets WHERE id = 'news-attachments') THEN
    RAISE NOTICE 'WARNING: Storage bucket "news-attachments" does not exist. Please create it manually.';
  ELSE
    RAISE NOTICE 'Storage bucket "news-attachments" exists: OK';
  END IF;
END $$;
...
-- Storage: Reusing existing buckets (news-images, news-attachments) ✓
-- ============================================================
